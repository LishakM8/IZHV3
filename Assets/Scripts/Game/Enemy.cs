...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        var currentEnemyPosition = currentEnemy.position;
        var target = GameManager.Instance.NearestPlayer(currentEnemyPosition);
        if (target == null) {
            return;
        }

        // Get the direction vector.
        var enemyToTarget = target.transform.position - currentEnemyPosition;
        
        // Allow only rotation in the XZ plane (play area).
        enemyToTarget.y = 0.0f;
        enemyToTarget.Normalize();
        
        // Rotate towards and move it.
        currentEnemy.rotation = Quaternion.LookRotation(enemyToTarget);
        currentEnemy.position += currentEnemy.rotation * new Vector3(0f, 0f, speed/100);
    }
